...
const getDepartment = async (event) => {
    try {
        const response = await axios.post(route('user.getDepartment'), {
            _method: 'post',
...
const submitForm = async () => {
    try {
        const response = await router.post(route('user.storee'), form);
        setTimeout(() => {
    ...
    <LayoutAuthenticated>
        ...
        <SectionMain>
            ...
            <CardBox :class="cardClass" class="my-2" form @submit.prevent="submit">
                ...
                <FormField label="No Telp" label-for="telp_number" help="Please enter your telp_number">
                    <FormControl v-model="form.telp_number" id="telp_number" :icon="mdiAccount"
                        autocomplete="telp_number" type="number" required />
                </FormField>

                <FormField label="Email" label-for="email" help="Please enter your email">
                    <FormControl v-model="form.email" id="email" :icon="mdiAccount" autocomplete="email" type="email"
                        required />
                </FormField>
                ...
                <form class="max-w-sm">
                    <label for="company_name"
                        class="block mb-2 mt-2 text-sm font-medium text-gray-900 dark:text-white">Select the company
                        name</label>
                    <select v-model="form.company_id" id="company" @change="getDepartment($event)"
                ...
                <form class="max-w-sm">
                    <label for="department_name"
                        class="block mb-2 mt-2 text-sm font-medium text-gray-900 dark:text-white">Select the department
                        name</label>
                    <select v-model="form.department_id" id="department"
                        class="bg-gray-50 border border-black text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                        <option v-for=" item in form.items" :value=item.id :key="item">{{ item.name }}</option>
                    </select>
                </form>

                <!-- <form class="max-w-sm">
            <label for="department_name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Select the department name</label>
            <select v-model="form.department_id" id="department" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                ...
                <form class="max-w-sm">
                    <label for="is_active"
                        class="block mb-2 mt-2 text-sm font-medium text-gray-900 dark:text-white">Select the user
                        status</label>
                ...
                <div class="form-control max-w-sm">
                    <label class="dy-cursor-pointer label">
                        <select v-model="form.role"
                            class="bg-gray-50 border border-black text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            <option v-for="(role, index) in roles" :key="index" :for="`check-${role.id}`" :value="role.name" :id="`check-${role.id}`">{{ role.name }}</option>
                        </select>
                    ...
                    <!-- <input class="dy-checkbox dy-checkbox-accent" type="checkbox" v-model="form.role" :value="role.name" :id="`check-${role.id}`">
            <label class="form-check-label ml-2" :for="`check-${role.id}`">{{ role.name }}</label> -->
                </div>
                <BaseDivider />
                <BaseButtons>
