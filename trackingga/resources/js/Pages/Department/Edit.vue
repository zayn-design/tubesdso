...
const submitForm = async () => {
  try {
    const response = await router.put(route('department.update', props.data.id), form);
    setTimeout(() => {
  ...
  <LayoutAuthenticated>
    ...
    <SectionMain class="grid grid-cols-1">
      ...
      >
        ...
        <form class="max-w-sm">
            <label for="is_active" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Select the department status</label>
            <select v-model="form.isActive" id="isActive" class="bg-gray-50 border border-black text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                <option disabled selected>Please select one</option>
