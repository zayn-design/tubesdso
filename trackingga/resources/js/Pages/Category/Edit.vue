...
const submitForm = async () => {
    try {
        const response = await router.put(route('category.update', props.data.id), form);
        setTimeout(() => {
    ...
    <LayoutAuthenticated>
        ...
        <SectionMain>
            ...
            <CardBox :class="cardClass" class="my-2" form @submit.prevent="submit">
                ...
                <FormField label="Category Name" label-for="name" help="Please enter the Category name">
                    <FormControl v-model="form.name" id="name" :icon="mdiAccount" autocomplete="name" type="text"
                        required />
                </FormField>
                <form class="max-w-sm">
                    <label for="company_name"
                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Select the category
                        status</label>
