...
const detail = (id) => {
    const response = router.get(route('trackin.edit', id))
}

const createPage = () => {
    const response = router.get(route('trackin.create'))
}

const filters = reactive({ ...props.filters });

const fetchData = (page) => {
    router.get(route('trackin.index', { page, search: filters.search, field: filters.field, direction: filters.direction }));
};
    ...
    <LayoutAuthenticated>
        ...
        <SectionMain>
            <SectionTitleLineWithButton :icon="mdiHomeCircle" title="Beranda" main>
                <BaseButtons @click="homePage()" type="justify-start lg:justify-end">
            ...
            <CardBox class="mb-6" has-table>
                <div class="m-5">
                    <div class="flex justify-between">
                        <BaseButtons v-if="can.create">
                            <BaseButton @click="createPage()" color="info" label="Tambah barang masuk" :icon="mdiPlus" />
                        </BaseButtons>
                        ...
                        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                            <thead
                                ...
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3" v-if="can.edit || can.create">Action</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr v-for="(data, index) in datas.data" :key="index"
                                    ...
                                    <td class="px-6 py-4" v-if="can.edit || can.create">
                                        <div class="inline-flex rounded-md shadow-sm" role="group">
                                            <BaseButtons>
                                                <BaseButton @click="detail(data.id)" color="info"
