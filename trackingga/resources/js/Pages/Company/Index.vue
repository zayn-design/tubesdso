...
const deleteData = async (id) => {
    try {
        const response = await router.delete(route('company.destroy', id));
        setTimeout(() => {
    ...
    <LayoutAuthenticated>
        ...
        <SectionMain>
            ...
            <CardBox class="mb-6" has-table>
                <div class="m-5">
                    <BaseButtons>
                        <BaseButton @click="createPage()" color="info" label="Tambah company"
                        :icon="mdiPlus"/>
                    </BaseButtons>
                    <div class="mt-5 overflow-x-auto shadow-md sm:rounded-lg">
                        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                            ...
                            <tbody>
                                <tr v-for="data in company" :key="data.id"
