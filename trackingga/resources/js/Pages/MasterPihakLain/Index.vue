...
const master_pihak_lain = ref([]);

const get_json_index = async (page = 1, field = '') => {

    if(field){
        filter.field_master_pihak_lain_list = field;
        filter.direction_master_pihak_lain_list = filter.direction_master_pihak_lain_list == 'asc' ? 'desc' : 'asc';
    }

    //HTTP request
    axios.get(`/sirkuler/master_pihak_lain/json_index?page=${page}&field=${filter.field_master_pihak_lain_list}&direction=${filter.direction_master_pihak_lain_list}&search=${filter.search_master_pihak_lain_list}&search_status=${(filter.search_status) ? filter.search_status.id : '' }`, {
    })
    .then(response => {

        master_pihak_lain.value = response.data.data;
...
const filter = reactive({
  field_master_pihak_lain_list: '',
  direction_master_pihak_lain_list: '',
  search_master_pihak_lain_list: '',
  search_status: '',
...
<template>
  <LayoutAuthenticated>
    <Head title="List Master Pihak Lain" />
    <SectionMain>
      <SectionTitleLineWithButton
        :icon="mdiFileDocumentEdit"
        title="List Master Pihak Lain"
        main
      >
